...
def vis( res, path_prefix, c=["firebrick", "forest", 'purple', 'salmon', "gold", "red"]):
    ...
    time.sleep(2)
    cmd.set("grid_mode", 0)
    #cmd.png(path_prefix + "_restraints.png", dpi=600)
    #cmd.delete("d*")
...
if(os.path.exists(out_stat_path)):
    f = open(out_stat_path, "rb")
...
for it in range(iterations):
    ...
    pairs = build_particle_clouds(out_dir=data_dir+"/iteration"+str(it), seedInt=random_seeds[it], max_particle=16, min_particles=15, particle_steps=10)  #Randomize point Clouds
